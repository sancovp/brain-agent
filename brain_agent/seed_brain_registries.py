...
def safe_add(registry_name: str, key: str, value_dict: dict):
    ...
    if "not found" not in existing:
        ...
        logger.info_print(f"Updated {key} in {registry_name}")
    else:
        registry_util_func(
            operation="add",
            registry_name=registry_name,
        ...
        logger.info_print(f"Added {key} to {registry_name}")
...
def main():
    ...
    logger.info_print(f"Personas: {persons}")
    logger.info_print(f"Modes: {modes}")
