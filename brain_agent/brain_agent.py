    ...
    try:
        ...
        try:
            ...
            if directory.startswith(heaven_data_dir + os.sep):
                ...
                logger.debug_print(f"Storing brain directory as relative path: {storage_directory}")
        except Exception as e:
            logger.debug_print(f"Could not resolve HEAVEN_DATA_DIR, storing absolute path: {e}")
        ...
        if "added to registry" in result:
            logger.info_print(f"Brain '{brain_name}' registered successfully with directory: {directory}")
        else:
            raise RuntimeError(f"Failed to register brain: {result}")
            
    except Exception as e:
        logger.log_exception(e, "register_brain")
        raise